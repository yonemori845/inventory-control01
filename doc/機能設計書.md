...
| image_path | text | Storage パスまたは公開 URL。**論理上は親商品あたり 1 枚**とし、同一 `product_group_id` の SKU では**同一文字列**を入れる運用を推奨（詳細・理想形は `doc/商品画像_収集と利用方針.md`） |
| is_active | boolean | |

**補足（商品画像）**：将来、`product_groups` に画像列を持たせ SKU は参照のみとする**理想形**がありうる。現行マイグレーションは `product_skus.image_path` のみ。CSV 取込フォーマットに画像列は**含めない**（`doc/在庫CSVとtemplate_DBの対応.md`）。
...
| Storage | オブジェクトキー `seed/...` 等。**本番・デモ用の推奨パス階層**は `doc/商品画像_収集と利用方針.md` §5。バケット設定は `doc/技術仕様書.md` §4.5 |
...
| 0.3.2 | 2026-04-05 | 商品画像：`image_path` の論理単位・CSV 非対応・Storage 参照を `商品画像_収集と利用方針` と整合。根拠を要件 v2.2 に同期 |
