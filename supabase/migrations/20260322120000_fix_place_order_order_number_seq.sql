...
begin
  ...
  select coalesce(max(
    case
      ...
      then substring(o.order_number from length(v_prefix) + 1)::int
    end
  ), 0)
  into v_seq
...
comment on function public.place_order(jsonb, numeric) is
  '注文確定: 在庫検証・orders/order_lines・在庫減・inventory_movements を 1 トランザクションで実行。税は pricing.ts と同じ丸め。連番は ORD-YYYYMMDD- の後がすべて数字の行のみ集計（非数字サフィックスは無視）。同日並行は advisory lock で直列化。';
